...
impl<I2C: I2c> Cst328<I2C> {
    ...
    pub fn read_touch(
        &mut self,
    ) -> Result<([TouchPoint; MAX_POINTS], usize), I2C::Error> {
    ...
    fn read_register(&mut self, register: u16, data: &mut [u8]) -> Result<(), I2C::Error> {
        self.i2c.write_read(self.addr, &register.to_be_bytes(), data)
    }
}

fn point_offset(index: usize) -> usize {
    if index == 0 { 0 } else { 7 + (index - 1) * BYTES_PER_POINT }
}
...
fn decode_point(data: &[u8]) -> TouchPoint {
    ...
    TouchPoint { x, y, pressure: data[4] }
}

const fn empty_point() -> TouchPoint {
    TouchPoint { x: 0, y: 0, pressure: 0 }
}
