fn mqtt_worker(conn: &mut EspMqttConnection, tx: Sender<UiInbound>) {
    loop {
        match conn.next() {
...
fn make_mqtt() -> anyhow::Result<(EspMqttClient<'static>, EspMqttConnection)> {
...
fn publish_outbound(client: &mut EspMqttClient<'_>, cmd: UiOutbound) -> anyhow::Result<()> {
    match cmd {
